...
######################################################################
## FIRST DO CHI2 BINARY FIT
######################################################################
print('-'*10)
print('FIRST DOING BINARY FIT')
print('-'*10)
## Do a chi2 fit for phases
params = Parameters()
params.add('ra',   value= dra2)
params.add('dec', value= ddec2)
params.add('ratio', value= a3_2, min=1.0)
params.add('ud1',   value= a4, vary=False)#min=0.0,max=2.0)
params.add('ud2', value= a5_2, vary=False)#min=0.0,max=2.0)
...
report_fit(result)
chi_sq_binary = result.redchi

print('-'*10)
print('NOW DOING TRIPLE SEARCH')
print('-'*10)
...
for ra_try in tqdm(ra_grid):
    ...
    if plot_grid == 'y':
        ...
        ax.scatter(ra_results,dec_results,c=chi_sq,cmap=cm.inferno)
        ax.invert_xaxis()
        #plt.colorbar()
        plt.draw()
        plt.pause(0.001)
...
## plot results
with PdfPages("/Users/tgardne/ARMADA_epochs/%(1)s/%(1)s_%(2)s_TRIPLE_detection.pdf"%{"1":target_id,"2":date}) as pdf:
    
    ## first page - chisq grid
    #plt.scatter(ra_results, dec_results, c=1/chi_sq, cmap=cm.inferno)
    plt.scatter(ra_results, dec_results, c=chi_sq, cmap=cm.inferno)
    plt.colorbar()
    plt.xlabel('d_RA (mas)')
    plt.ylabel('d_DE (mas)')
