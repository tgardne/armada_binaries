...
def add_planet(period,wobble,bigomega,inc,t0,epochs):
    omega = 0
    ecc = 0
    bigomega = bigomega*np.pi/180
    inc = inc*np.pi/180
    #Thiele-Innes elements for secondary
    A = wobble*(np.cos(bigomega)*np.cos(omega)-np.sin(bigomega)*np.cos(inc)*np.sin(omega))
    B = wobble*(np.sin(bigomega)*np.cos(omega)+np.cos(bigomega)*np.cos(inc)*np.sin(omega))
    F = wobble*(-np.cos(bigomega)*np.sin(omega)-np.sin(bigomega)*np.cos(inc)*np.cos(omega))
    G = wobble*(-np.sin(bigomega)*np.sin(omega)+np.cos(bigomega)*np.cos(inc)*np.cos(omega))
    #Calculate the mean anamoly for each t in dataset:
    M = []
    for i in epochs:
        m_anom=2*np.pi/period*(i-t0)
        M.append(m_anom)
    M=np.asarray(M)
    #eccentric anamoly calculated for each t (using kepler function):
    E=[]
    for j in M:
        e_anom=ks.getE(j,ecc)
        E.append(e_anom)
    E=np.asarray(E)
