...
wds_file = '/Users/tgardner/ARMADA_isochrones/WDS_Data/WDS_Data_with_methods_added/'
...
Target_List = ['158140','78316','129246']
...
for target_hd in Target_List:
    ...
    for feh in feh_set:
        #try:
            ...
            for distance in distance_set:
                ...
                age_grid = np.linspace(6, 10, 50)  ## do fewer steps to go faster
    ...
    for i in [4]:
            ...
            if (np.min(comb_xvals) < np.nanmedian(all_modelx_best[i])) == True:
                x_axis_max = np.nanmedian(all_modelx_best[i]) + 0.15
                idx_xmax = comb_xvals.index(np.nanmax(comb_xvals))
                idx_xmin = comb_xvals.index(np.nanmin(comb_xvals))
                print(idx_xmin)
                print(comb_xvals)
                print(comb_err_xvals)
                x_axis_min =np.nanmin(comb_xvals) - comb_err_xvals[idx_xmin] - 0.1- comb_err_xvals[idx_xmin] - 0.1
            elif (np.min(comb_xvals) < np.nanmedian(all_modelx_best[i])) == False and np.isnan(comb_xvals[1]) == False:
                idx_xmax = comb_xvals.index(np.nanmax(comb_xvals))
                x_axis_max = np.nanmax(comb_xvals) + comb_err_xvals[idx_xmax] + 0.1
                x_axis_min = np.nanmedian(all_modelx_best[i]) - 0.15
            elif np.isnan(comb_xvals[1]) == True:
                x_axis_min = np.nanmin(all_modelx_best[i])-0.15
                x_axis_max = np.nanmax(all_modelx_best[i]) - 0.15
            print(x_axis_min,x_axis_max)
