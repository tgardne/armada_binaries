...
##
## Inject random astrometric jitter and see how it effects periodogram
##
######################################################################
...
def add_noise(epochs,noise_amp,direction='random'):
    noise_amp = np.random.uniform(-noise_amp,noise_amp,size=len(epochs))
    if direction=='random':
        noise_angle = np.random.uniform(0,360,size=len(epochs))*np.pi/180
    else:
        noise_angle = direction*np.pi/180

    xx = noise_amp*np.sin(noise_angle)
    yy = noise_amp*np.cos(noise_angle)
    return(xx,yy)
...
## ADD RANDOM NOISE
############################################
print('ADDING ASTROMETRIC NOISE')
noise_amp = float(input('Amplitude of random noise (uas): '))/1000
noise_angle = input('Angle of noise amplitude (deg) [Enter n for random angle]: ')

if noise_angle=='n':
    noise_xy_all = add_noise(t_all,noise_amp)
else:
    noise_xy_all = add_noise(t_all,noise_amp,float(noise_angle))

xpos += noise_xy_all[0][:len(t)]
ypos += noise_xy_all[1][:len(t)]
xpos_wds += noise_xy_all[0][len(t):]
ypos_wds += noise_xy_all[1][len(t):]
xpos_all += noise_xy_all[0]
ypos_all += noise_xy_all[1]
...
ax.set_title('HD%s Outer Orbit'%target_hd)
plt.legend()
plt.savefig('%s/HD%s_%s_outer_leastsquares_noise.pdf'%(directory,target_hd,note))
plt.close()
...
plt.savefig('%s/HD%s_%s_resid_leastsquares_noise.pdf'%(directory,target_hd,note))
plt.close()
...
f = open("%s/%s_orbit_ls_noise.txt"%(directory,target_hd),"w+")
...
## Save txt file with noisy epochs
p_noise=[]
theta_noise = []
for i,j in zip(xpos_all[:len(xpos)],ypos_all[:len(xpos)]):
    pnew,tnew = cart2pol(i,j)
    p_noise.append(pnew)
    theta_noise.append(tnew)
p_noise = np.array(p_noise)
theta_noise = np.array(theta_noise)
f = open("%s/HD_%s_noise_%s.txt"%(directory,target_hd,note),"w+")
f.write("# date mjd sep pa err_maj err_min err_pa\r\n")
for i,j,k,l,m,n in zip(t,p_noise,theta_noise,error_maj,error_min,error_deg):
    f.write("-- %s %s %s %s %s %s\r\n"%(i,j,k,l,m,n))
f.write('#')
f.close()
...
for period in tqdm(P2):
    ...
    for i in np.arange(10):
        ...
        params.add('a2', value= resids_median/1000, min=0)
...
plt.savefig('%s/HD%s_%s_chi2_period_noise.pdf'%(directory,target_hd,note))
plt.close()
...
plt.savefig('%s/HD%s_%s_semi_inc_grid_noise.pdf'%(directory,target_hd,note))
plt.close()